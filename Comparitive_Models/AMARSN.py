...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride):
        ...
        self.multiscaleCNN = Multiscale_CNN2(in_channel, out_channel, stride)
...
class Multiscale_CNN2(nn.Module):
    def __init__(self, in_channel, out_channel, stride):
        ...
        self.wideconv = nn.Conv1d(in_channel, out_channel, kernel_size=32, stride=stride, padding=15, bias=False)
        self.conv1 = nn.Conv1d(out_channel, out_channel, kernel_size=7, stride=1, padding=3, bias=False)

        self.conv2_1 = nn.Conv1d(out_channel, out_channel, kernel_size=7, stride=1, padding=3, bias=False)
        self.conv2_2 = nn.Conv1d(out_channel, out_channel, kernel_size=5, stride=2, padding=2, bias=False)

        self.conv3_1 = nn.Conv1d(out_channel, out_channel, kernel_size=7, stride=1, padding=3, bias=False)
        self.conv3_2 = nn.Conv1d(out_channel, out_channel, kernel_size=5, stride=1, padding=2, bias=False)
        self.conv3_3 = nn.Conv1d(out_channel, out_channel, kernel_size=3, stride=2, padding=1, bias=False)

        self.maxpool = nn.MaxPool1d(kernel_size=3, stride=1, padding=1)
        self.conv4 = nn.Conv1d(in_channel, out_channel, kernel_size=1, stride=2, bias=False)
        self.sa = Self_Attention()
    def forward(self, input):
        x0 = self.wideconv(input) #(64,4,1024)
        x1 = self.conv1(x0) #(64,4,1024)

        x0_2 = torch.cat([x0, x1], dim=2) #(64,4,2048)
        x2_1 = self.conv2_1(x0_2) #(64,4,1024)
        x2_2 = self.conv2_2(x2_1) #(64,4,1024)

        x0_3 = torch.cat([x0, x2_2], dim=2) #(64,4,2048)
        x3_1 = self.conv3_1(x0_3) #(64,4,2048)
        x3_2 = self.conv3_2(x3_1) #(64,4,2048)
        x3_3 = self.conv3_3(x3_2) #(64,4,1024)

        x0_4 = torch.cat([x0, x3_3], dim=2) #(64,4,2048)
        x4_1 = self.maxpool(x0_4) #(64,4,2048)
        x4_2 = self.conv4(x4_1) #(64,4,1024)
...
class Shrinkage_Gap(nn.Module):
    def __init__(self, channel, gap_size):
        ...
        self.GAP = nn.AvgPool1d(kernel_size=64, stride=64)
        self.gap = nn.AdaptiveAvgPool1d(gap_size)
        self.fc = nn.Sequential(
            nn.Linear(channel, channel),
            nn.BatchNorm1d(channel),
